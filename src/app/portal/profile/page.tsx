...
export default async function CustomerProfilePage() {
  ...
  const customer = await prisma.customer.findUnique({
    ...
    include: {
      user: {
        select: { email: true, firstName: true, lastName: true, passwordSetAt: true },
      },
      organization: {
  ...
  async function updatePasswordAction(formData: FormData) {
    'use server';
    const { updateCustomerPasswordFromProfile } = await import('@/app/actions/customer-auth');
    const res = await updateCustomerPasswordFromProfile(formData);
    if (!res.success) {
        ...
        <div className="md:col-span-1 space-y-6">
          ...
          <div className="bg-white border border-neutral-200 rounded-2xl p-6 shadow-sm">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h3 className="font-semibold text-neutral-900">Security & Password</h3>
                <p className="text-xs text-neutral-500">
                  {customer.user.passwordSetAt ? 'Permanent password active' : 'No permanent password set'}
                </p>
              </div>
            </div>

            <form action={updatePasswordAction} className="space-y-3">
              {customer.user.passwordSetAt && (
                <div>
                  <label htmlFor="currentPassword" className="block text-xs font-semibold text-neutral-700 mb-1">
                    Current Password
                  </label>
                  <input
                    id="currentPassword"
              ...
              <div>
                <label htmlFor="newPassword" className="block text-xs font-semibold text-neutral-700 mb-1">
                  {customer.user.passwordSetAt ? 'New Password' : 'Create Permanent Password'}
                </label>
                <input
                  id="newPassword"
                  type="password"
              ...
              <button
                type="submit"
                className="w-full py-2 bg-slate-900 hover:bg-slate-800 text-white font-medium rounded-lg text-sm transition"
              >
                {customer.user.passwordSetAt ? 'Update Password' : 'Set Permanent Password'}
              </button>
            </form>
          </div>
